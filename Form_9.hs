...
simplify (Not p)    =  pushNot (simplify p)
  ...
  pushNot (p :<->: q) = (p :&: pushNot q) :|: (pushNot p :&: q)
  pushNot p          =  Not p
...
prop_simplify_sound' p =
    forAll (vector $ length v) $ \bs ->
    let val = zip v bs in eval val p == eval val (simplify p)
  where v = vars p
